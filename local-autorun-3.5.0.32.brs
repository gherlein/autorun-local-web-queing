...
Sub Main()
    ...
    autorunVersion$ = "6.5.28" ' BA 3.5.0.32
    customAutorunVersion$ = "6.5.0"
...
' check to see whether or not the current firmware meets the minimum compatibility requirements
	...
	if sysInfo.deviceFamily$ = "panther" then
		minVersionNumber% = 263212
		minVersion$ = "4.4.44"
	else if sysInfo.deviceFamily$ = "cheetah" then
		minVersionNumber% = 263212
		minVersion$ = "4.4.44"
	else
		minVersionNumber% = 199189
		minVersion$ = "3.10.21"
	endif
...
Sub RunBsp(sysFlags As Object, sysInfo As Object, diagnosticCodes As Object)
	...
	BSP.stripTwitterUserNames = true
...
' create objects for all attached button panels
    ...
    if BSP.registrySettings.lwsConfig$ = "c" or BSP.registrySettings.lwsConfig$ = "s" then
        ...
        BSP.GetConfigurationPageAA ={ HandleEvent: GetConfigurationPage, mVar: BSP }
		BSP.SetValuesAA =			{ HandleEvent: SetValues, mVar: BSP }
        ...
        BSP.GetWaitListAA ={ HandleEvent: GetWaitListPage, mVar: BSP }
        BSP.localServer.AddGetFromEvent({ url_path: "/WaitList", user_data: BSP.GetWaitListAA})
...
Sub StartPlayback()
	...
	' unmute all audio explicitly for Cheetah / Panther
	m.UnmuteAllAudio()
...
Function GetCurrentUserVariableValue() As String
...
Sub ReadVariablesDB()
	...
	selectStmt = m.userVariablesDB.CreateStatement("SELECT Sections.SectionName, Variables.VariableName, Variables.CurrentValue, Variables.DefaultValue FROM Variables INNER JOIN Sections ORDER BY Sections.SectionName;")
...
' get the item
    ...
    if stateXML.imageItem.Count() = 1 then
		...
		newRFInputPlaylistItem(stateXML.rfInputItem, state)
        ...
        newRSSImagePlaylistItem(stateXML.rssImageItem, state)
        state.type$ = "mediaRSS"
        ...
        newVideoStreamPlaylistItem(stateXML.streamItem, state)
        state.type$ = "stream"
		...
		newHtml5PlaylistItem(stateXML.html5Item, state)
		state.type$ = "html5"
...
Sub newHtml5PlaylistItem(playlistItemXML As Object, state As Object)

	state.name$ = playlistItemXML.name.GetText()
	
	contentIsLocal$ = playlistItemXML.contentLocal.GetText()
	if lcase(contentIsLocal$) = "true" then
		state.contentIsLocal = true
	else
		state.contentIsLocal = false
	endif

	state.prefix$ = playlistItemXML.prefix.GetText()
	state.filePath$ = playlistItemXML.filePath.GetText()
	state.url$ = playlistItemXML.url.GetText()
...
Sub newRFInputPlaylistItem(playlistItemXML As Object, state As Object)

	state.channelName$ = playlistItemXML.rfInputChannel.channelName.GetText()
	state.frequency$ = playlistItemXML.rfInputChannel.frequency.GetText()
	state.encoding$ = playlistItemXML.rfInputChannel.encoding.GetText()
	state.videoPID$ = playlistItemXML.rfInputChannel.videoPID.GetText()
	state.audioPID$ = playlistItemXML.rfInputChannel.audioPID.GetText()
...
Sub newIndexedLiveTextDataEntryTemplateItem(bsp As Object, templateItems As Object, templateItemXML As Object)
	...
	templateItem.index% = int(val(index$))
...
Sub newTitledLiveTextDataEntryTemplateItem(bsp As Object, templateItems As Object, templateItemXML As Object)
	...
	templateItem.title$ = templateItemXML.title.GetText()
...
Sub newRSSImagePlaylistItem(playlistItemXML As Object, state As Object)
...
Sub newMediaListPlaylistItem(bsp As Object, zoneHSM As Object, playlistItemXML As Object, state As Object)
	...
	state.ConfigureMediaListNavigationButton = ConfigureMediaListNavigationButton
	state.LaunchAudio = LaunchAudio
	state.LaunchMixerAudio = LaunchMixerAudio
	state.LaunchVideo = LaunchVideo
	state.DisplayImage = DisplayImage
	state.StartInactivityTimer = StartInactivityTimer
	state.HandleMediaListNavigation = HandleMediaListNavigation
...
Sub newVideoStreamPlaylistItem(playlistItemXML As Object, state As Object)

	videoStreamSpecAttrs = playlistItemXML.streamSpec.GetAttributes()
	state.url$ = videoStreamSpecAttrs["url"]

    state.HStateEventHandler = STVideoStreamPlayingEventHandler
	state.MediaItemEventHandler = MediaItemEventHandler
    state.ConfigureBPButtons = ConfigureBPButtons
    state.LaunchTimer = LaunchTimer
	state.ExecuteTransition = ExecuteTransition
...
Function newTwitterPlaylistItem(bsp As Object, zoneHSM As Object, playlistItemXML As Object) As Object
	...
	url = newTextParameterValue(twitterSpecAttrs["url"])
...
Sub MuteAudioOutputs(muteOn as Boolean, parameters As Object)
	...
	parameter = parameters["analog"]
	analogMute$ = parameter.GetCurrentParameterValue()
	if lcase(analogMute$) = "true" then
		analogAudioOutput = CreateObject("roAudioOutput", "Analog")
		analogAudioOutput.SetMute(muteOn)
	endif

	parameter = parameters["hdmi"]
	hdmiMute$ = parameter.GetCurrentParameterValue()
	if lcase(hdmiMute$) = "true" then
		hdmiAudioOutput = CreateObject("roAudioOutput", "HDMI")
		hdmiAudioOutput.SetMute(muteOn)
	endif

	parameter = parameters["spdif"]
	spdifMute$ = parameter.GetCurrentParameterValue()
	if lcase(spdifMute$) = "true" then
		spdifAudioOutput = CreateObject("roAudioOutput", "SPDIF")
		spdifAudioOutput.SetMute(muteOn)
	endif
...
Sub ChangeConnectorVolume(multiplier% As Integer, parameters As Object)
	...
	if lcase(connector$) = "analog" then
		m.analogVolume% = ExecuteChangeConnectorVolume("Analog", m.analogVolume% + volumeDelta%)
	else if lcase(connector$) = "hdmi" then
...
Sub ConfigureAudioResources()
	...
	if m.bsp.sysInfo.deviceFamily$ = "panther" or m.bsp.sysInfo.deviceFamily$ = "cheetah" then
...
Sub newZoneCommon(bsp As Object, zoneXML As Object, zoneHSM As Object)
	...
	zoneHSM.ShowImagePlane = ShowImagePlane
...
Function MediaItemEventHandler(event As Object, stateData As Object) As Object
    ...
    if type(event) = "roControlDown" and stri(event.GetSourceIdentity()) = stri(m.bsp.controlPort.GetIdentity()) then
        ...
        if type(keyboardEvents) = "roAssociativeArray" then        
			if type(keyboardEvents["<any>"]) = "roAssociativeArray" then
				keyboardChar$ = "<any>"
			else
...
Sub PreloadItem()
    ...
    if type(m.imageItem) = "roAssociativeArray" then
        imageItem = m.imageItem
...
Sub SetAudioOutputAndMode(player As Object)
	...
	if m.bsp.sysInfo.modelSupportsRoAudioOutput and m.presentationUsesRoAudioOutputParameters and m.bsp.currentPresentationUsesRoAudioOutputParameters then
		...
		if lcase(m.analogOutput$) <> "none" then
			pcm.push(analogAudioOutput)
		endif
...
Function STVideoStreamPlayingEventHandler(event As Object, stateData As Object) As Object
    ...
    if type(event) = "roAssociativeArray" then      ' internal message event
        ...
        if IsString(event["EventType"]) then
            ...
            if event["EventType"] = "ENTRY_SIGNAL" then
				...
				if ok = 0 then
					m.bsp.diagnostics.PrintDebug("Error playing rtsp file in STVideoStreamPlayingEventHandler: url = " + m.url$)
				...
				' state logging
				m.bsp.logging.WriteStateLogEntry(m.stateMachine, m.id$, "videoStream")

				' playback logging
				m.stateMachine.LogPlayStart("videoStream", m.url$)
...
'				m.stateMachine.LogPlayStart("video", "mjpeg")
			endif
        endif
    else
        return m.MediaItemEventHandler(event, stateData)
...
Sub ConfigureMediaListNavigationButton(navigation As Object)
    ...
    if type(event) = "roAssociativeArray" then      ' internal message event
        ...
        if IsString(event["EventType"]) then
            ...
            if event["EventType"] = "ENTRY_SIGNAL" then
				...
				m.ConfigureMediaListNavigationButton(m.nextNavigation)
				m.ConfigureMediaListNavigationButton(m.previousNavigation)
...
'				m.AdvanceMediaListPlayback(false)
	...
	if type(m.nextNavigation) = "roAssociativeArray" then
		advance = m.HandleMediaListNavigation(event, m.nextNavigation)
		if advance then
			m.AdvanceMediaListPlayback(true)
			return "HANDLED"
		endif
	...
	if type(m.previousNavigation) = "roAssociativeArray" then
		retreat = m.HandleMediaListNavigation(event, m.previousNavigation)
		if retreat then
...
Function HandleMediaListNavigation(event As Object, navigation As Object) As Boolean
    ...
    if type(event) = "roAssociativeArray" then      ' internal message event
        ...
        if IsString(event["EventType"]) then
            ...
            if event["EventType"] = "ENTRY_SIGNAL" then
				...
				if type(m.volume%) = "roInt" then
					m.stateMachine.videoPlayer.SetVolume(m.volume%)
				endif

				m.stateMachine.videoPlayer.EnableSafeRegionTrimming(true)
				
				aa=CreateObject("roAssociativeArray")
'				aa["ChannelConfig"] = "KNTV-HD:207000000:QAM_256:257:258:12"
				aa["ChannelConfig"] = m.channelName$ + ":" + m.frequency$ + ":" + m.encoding$ + ":" + m.videoPID$ + ":" + m.audioPID$ + ":0"
				m.stateMachine.videoPlayer.PlayFile(aa)
				...
				' state logging
				m.bsp.logging.WriteStateLogEntry(m.stateMachine, m.id$, "atsc")

				' playback logging
				m.stateMachine.LogPlayStart("atsc", m.channelName$)
...
'				m.touchScreen.EnableCursor(true)
				...
				m.html = CreateObject("roHtmlWidget", m.stateMachine.rectangle)
				m.html.SetPort(m.bsp.msgPort)

				m.html.EnableSecurity(not m.enableExternalData)
				m.html.EnableMouseEvents(m.enableMouseEvents)
				...
				if m.contentIsLocal then
					...
					stateName$ = m.name$
					prefix$ = m.prefix$
	'				prefix$ = presentationName$ + "-" + stateName$ + "-"

					m.html.MapFilesFromAssetPool(assetPool, assetCollection, prefix$, "/" + prefix$ + "/")
					m.html.SetURL("file:///" + prefix$ + "/" + m.filePath$)
...
Sub SetupTemplateMRSS()
	...
	m.bsp.feedPlayer = newFeedPlayer(m.stateMachine, m.stateMachine.imagePlayer, invalid, m.bsp.msgPort, m.bsp.feedVideoDownloader, false, mrssFeedUrl$, 0, m.bsp.diagnostics, false)
    ...
    if type(event) = "roAssociativeArray" then      ' internal message event
        ...
        if IsString(event["EventType"]) then
            ...
            if event["EventType"] = "ENTRY_SIGNAL" then
				...
				if type(m.stateMachine.imagePlayer) = "roImageWidget" then
					m.stateMachine.imagePlayer.Hide()
				endif

				m.stateMachine.canvasWidget.Show()
...
Sub TemplateFeedRetrieved()
	...
	m.bsp.feedplayer.addSecsToTimer (m.bsp.feedplayer.feed.ttlminutes * 60, m.bsp.feedplayer.feedTimer)
        ...
        if IsString(event["EventType"]) then
            ...
            if event["EventType"] = "ENTRY_SIGNAL" then
				...
				if type(m.bsp.feedPlayer) <> "roAssociativeArray" then
					...
					m.bsp.feedPlayer = newFeedPlayer(m.stateMachine, m.stateMachine.imagePlayer, videoPlayer, m.bsp.msgPort, m.bsp.feedVideoDownloader, loopMode, m.rssURL$, slideTransition%, m.bsp.diagnostics, m.isDynamicPlaylist)
					if not m.bsp.feedPlayer.FetchFeed() then
						m.numFeedFetchRetries = m.numFeedFetchRetries + 1
						m.RestartFetchFeedTimer()
					endif
	...
	for each rssDataFeedItem in m.stateMachine.rssDataFeedItems
		if type(rssDataFeedItem.textStrings) = "roArray" then
			...
			for each article in rssDataFeedItem.liveDataFeed.articles
				if rssDataFeedItem.isTwitterFeed and m.bsp.stripTwitterUserNames and rssDataFeedItem.twitterUserName$ <> "" then
					article = Mid(article, len(rssDataFeedItem.twitterUserName$))
				endif
...
Function ExecuteSwitchPresentationCommand(presentationName$ As String) As Boolean
	...
	switchPresentationEvent["Presentation"] = presentationName$
...
REM 
Function newFeedPlayer(stateMachine As Object, imagePlayer As Object, videoPlayer As Object, msgPort As Object, videoDownloader As Object, loopMode As Object, rssURL$ As String, slideTransition% As Integer, diagnostics As Object, isDynamicPlaylist As Boolean) As Object
	fp = { name:"FeedPlayer" }
...
Function pFeedPlayer_HandleTimerEvent( event As Object ) As Void
	...
	if eventId = m.imageTimer.GetIdentity() then
		...
		if not m.fetchfeed() then
			...
			m.diagnostics.PrintDebug("Successful return from fetch feed, reseting timer to:" + stri(m.feed.ttlMinutes) + " minutes")
...
' returns a Dish object from which you can get averything about the subsystem'
Function initExtra(bsp as Object,mp as Object) as Object
...
Sub PopulateUserVarData(mVar As Object, root As Object)
  ...
  for each variableName in varList
    userVariable = userVariables.Lookup(variableName)
    elem = root.AddElement(variableName)
...
' print "respond to GetConfigurationPage request"
  ...
  q=ReadAsciiFile("SD:/local/index.html")

  e.SetResponseBodyString(q)
...
Sub GetConfigurationPageOld(userData as Object, e as Object)
...
' print "respond to GetConfigurationPage request"
  ...
  nc = CreateObject("roNetworkConfiguration", 0)
    ...
    for each variableName in varList
      userVariable = userVariables.Lookup(variableName)
      s = s + variableName + ":<br>" + Chr(13) + Chr(10)
      s = s + "<input type=" + chr(34) + "text" + chr(34) + " name=" + chr(34) + variableName + chr(34) + " value=" + chr(34)
      s = s + userVariable.GetCurrentValue() + chr(34) + " size=" + chr(34) + "100" + chr(34) + " /> <br />" + Chr(13) + Chr(10)
    next

  q=ReadAsciiFile("SD:/local/index.html")

  r = CreateObject("roRegex", "USERVARS", "i")
  s=r.ReplaceAll(q,s)

  r2 = CreateObject("roRegex", "PLAYERIPADDR", "i")
  s2=r2.ReplaceAll(s,ipAddress$)

  e.SetResponseBodyString(s)
  if not e.SendResponse(200) then stop
...
'******************************************************
'Insertion Sort - cribbed from Roku - https://github.com/sunlightlabs/Roku-Supreme-Court/blob/master/source/generalUtils.brs
...
Sub GetWaitListPage(userData as Object, e as Object)
...
' print "respond to GetConfigurationPage request"
  ...
  q=ReadAsciiFile("SD:/local/waitlist.html")
...
Sub GetWaitListVars(userData as Object, e as Object)
    ...
    root = CreateObject("roXMLElement")
    root.SetName("WaitListVariables")

    PopulateWaitListData(mVar, root)
...
Sub PopulateWaitListData(mVar As Object, root As Object)
...
Sub ActWaitListVars(userData as Object, e as Object)
  ...
  if act="get"
    ...
    name=args.name.trim()
    number=args.number.trim()
    print "change request: "+name+" to: "+number
    num=mVar.waitlistNames.Count()
    x=0
    while x<num
      print "matching ["+name+"] against ["+mVar.waitlistNames[x]+"]"
      if name=mVar.waitlistNames[x]
        mVar.waitlistSizes[x]=number
      end if
      x=x+1
    end while
  else if act="delete"
